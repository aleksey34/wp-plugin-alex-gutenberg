  ...
  */
    ...
    export default function Edit({attributes:{title} , setAttributes}) {
	 const tags = useSelect( ( select ) =>
		 select( 'core' ).getEntityRecords( 'taxonomy', 'post_tag', // may be -category and others
             ...
             <div { ...useBlockProps() } style={{display:"flex", flexWrap:'wrap'}}>
				<div className="sidebar_wrap">
					<div className="side_bar_heading">
						<h6>{ title ? title : 'Explore tags' } </h6>
					</div>
						...
						<ul>
							{
								tags && tags.map( (tag)=>{
									return (
										<li key={tag.id}><a href={tag.link}> {tag.name} </a></li>
									)
								})
							}
						</ul>
